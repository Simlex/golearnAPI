...
const allPublishers = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .json({ success: true, count: publishers.length, data: publishers });
});
...
const getPublisher = asyncHandler(async (req, res, next) => {
  ...
  if (!publisher)
    return next(new ErrorResponse(`No Publisher with the id ${id}`, 404));
  if (publisher.role != "publisher")
    return next(new ErrorResponse("This user is not a publisher", 400));
