...
const register = asyncHandler(async (req, res, next) => {
  let { firstName, lastName, userName, email, password, role, phoneNumber } =
    req.body;
  ...
  if (checkMail)
    return next(
      new ErrorResponse(`${email} has already been used by another user`, 400)
    );
  ...
  if (checkUserName)
    return next(
      new ErrorResponse(
        `${userName} has already been used by another user`,
        400
      )
    );
...
const generateToken = asyncHandler(async (req, res, next) => {
  ...
  if (!user)
    return next(new ErrorResponse(`user with ${email} does not exist`, 404));
  ...
  // send token to email
  try {
    sendMail(user.email, "click on this link to reset your password", message);
  } catch (error) {
  ...
  res.status(200).json({
    success: true,
    msg: "We have sent you an email to reset your password. Please check your email and follow the directions provided to retrieve your password.",
  });
});
...
const uploadDisplayPicture = asyncHandler(async (req, res, next) => {
  ...
  const uploader = async (path) =>
    await cloudinary.uploads(path, "displaypicture");
  ...
  res.status(200).json({
    success: true,
    msg: "successfully uploaded display picture",
    data: user,
  });
});
